...
- **⚡ 纯静态、零后端** — 前端直连公共 API（都支持 CORS），可部署到 Vercel/任意静态托管，无数据库、无服务器、无 API key。
- **🎨 OpenAlex 风格界面** — 大搜索框 + facet 过滤（厂商/状态/认证/原理）+ 列表式结果 + 结果计数；**中英文切换**、深浅主题、响应式。
- **🤖 可选 LLM 精准消歧** — 配上 OpenAI key（服务端 Serverless 函数，key 不进前端）即可对证据句做语义复核，过滤"通用型号名"的误报；不配也能用（自动降级到启发式）。见下方「可选：LLM 消歧」。
...
纯静态版用启发式消歧（方法节域检索 + 品牌词共现），已能过滤大部分误报。要更高精度，可启用 `api/disambiguate.js`（Vercel Serverless 函数）——它用 OpenAI 对证据句做语义复核，剔除"通用型号名"的巧合匹配（如激光波长 405 nm）。**key 只存在服务端环境变量，绝不进前端代码或 git。**

```bash
# 1) 在 platform.openai.com 生成一个 API key
# 2) 配置到 Vercel（服务端环境变量）
vercel env add OPENAI_API_KEY production
# 3) 部署（本地构建带 Serverless 函数，再上传）
vercel build --prod && vercel deploy --prebuilt --prod
```

没配 key 时函数返回 503，前端自动降级到启发式——所以**不启用也完全可用**。启用后，型号详情页会显示 `✓ LLM` 标记，误报被过滤。

> 部署带函数必须用 `vercel build --prod && vercel deploy --prebuilt`（本地构建）；纯 `vercel --prod` 的远程构建只出静态站（前端降级，无 LLM）。
